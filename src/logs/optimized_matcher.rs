...
fn decode_base64_discriminator(trimmed: &str) -> Option<u64> {
    ...
    let decoded_len = {
        ...
        let decoded = base64_simd::STANDARD
            .decode(&bytes[..12], discriminator_buf.as_mut().as_out())
            .ok()?;
        decoded.len()
    };
    if decoded_len < 8 {
...
fn parse_unscoped_pumpfun_launchlab_trade(
    ...
    let wants_pumpfun = event_type_filter
        .map(filter_wants_pumpfun_trade_event)
        .unwrap_or(true);
    let wants_launchlab = event_type_filter
        .map(filter_wants_launchlab_trade_event)
        .unwrap_or(true);
...
pub fn detect_pumpfun_create(logs: &[String]) -> bool {
    logs.iter()
        .any(|log| PUMPFUN_CREATE_FINDER.find(log.as_bytes()).is_some())
}
...
mod tests {
    ...
    fn large_program_data_uses_heap_fallback_without_dropping_event() {
        ...
        assert!(
            encoded.len() > 2700,
            "test must exceed the old fixed stack buffer limit"
        );
        ...
        match event {
            DexEvent::PumpFeesCreateFeeSharingConfig(event) => {
                ...
                assert_eq!(
                    event.status,
                    crate::core::events::PumpFeesConfigStatus::Active
                );
            }
